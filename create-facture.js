...
function init() {
    ...
    document.getElementById('factureDate').valueAsDate = today;
    updateValidityDate();
    
    // Event listener pour recalculer la date de validité
    document.getElementById('factureDate').addEventListener('change', updateValidityDate);
    ...
    // Charger les données si on génère depuis un devis
    loadDevisDataIfExists();
}

// Charger les données d'un devis si disponibles
function loadDevisDataIfExists() {
    const devisDataStr = sessionStorage.getItem('generateFactureFromDevis');
    if (!devisDataStr) return;
    
    try {
        const devisData = JSON.parse(devisDataStr);
        console.log('📋 Génération de facture depuis devis:', devisData.devisNumber);
        
        // Pré-sélectionner le client
        if (devisData.type) {
            selectedClient = {
                type: devisData.type,
                companyName: devisData.companyName,
                siren: devisData.siren,
                numTVA: devisData.numTVA,
                email: devisData.email,
                adresse: devisData.adresse,
                codePostal: devisData.codePostal,
                ville: devisData.ville,
                pays: devisData.pays,
                contactPrenom: devisData.contactPrenom,
                contactNom: devisData.contactNom,
                contactTelephone: devisData.contactTelephone,
        ...
        if (devisData.items && Array.isArray(devisData.items)) {
            factureItems = [...devisData.items];
            displayItems();
            updateTotals();
        }
        ...
        // Charger les notes
        if (devisData.notes) {
            document.getElementById('notes').value = devisData.notes;
        }
        
        // Charger l'acompte si présent
        if (devisData.acompteRequired) {
            document.getElementById('acompteCheck').checked = true;
            document.getElementById('acompteInputWrapper').style.display = 'block';
            document.getElementById('acomptePercent').value = devisData.acomptePercent || 30;
        }
        
        // Nettoyer le sessionStorage
        sessionStorage.removeItem('generateFactureFromDevis');
        
        showSuccessMessage('✓ Données du devis chargées !');
...
function updateValidityDate() {
    const factureDate = document.getElementById('factureDate').value;
    ...
    document.getElementById('echeanceDate').value = `${year}-${month}-${day}`;
}
...
function setupEventListeners() {
    ...
    document.getElementById('generateFacture')?.addEventListener('click', generateFacture);
...
function goToStep(step) {
    ...
    if (step === 3 && factureItems.length === 0) {
...
function addItemToDevis() {
    ...
    showSuccessMessage('✓ Article ajouté à la facture !');
}
...
function updateTotals() {
    const totalHT = factureItems.reduce((sum, item) => sum + item.total, 0);
...
function displayReviewSummary() {
    ...
    reviewSummary.innerHTML = `
        ...
        <div style="margin-bottom: 15px;">
            <strong>${factureItems.length} article(s)</strong>
        </div>
        <div class="total-row">
            <span>Total HT</span>
            <span>${formatAmount(totalHT)}</span>
...
async function generateFacture() {
    ...
    // Générer le numéro de facture selon le format
    ...
    const factureData = {
        ...
        echeanceDate: document.getElementById('echeanceDate').value,
...
function generatePDF(data) {
    ...
    doc.text('N° ' + data.factureNumber, 105, 28, { align: 'center' });
    ...
    // Date
    doc.text('Date: ' + data.factureDate, 20, 70);
    ...
    doc.setFont(undefined, 'normal');
    doc.setTextColor(0, 0, 0);
    doc.text('À régler avant le: ' + data.echeanceDate, 20, y);
    ...
    doc.save(`Facture_${clientName.replace(/\s/g, '_')}_${data.factureDate}.pdf`);
}
...
async function loadCompanyInfo() {
    ...
    try {
        ...
        if (!querySnapshot.empty) {
            ...
            console.log('📋 Format facture:', companyInfo.factureFormat);
...
// Générer le numéro de facture selon le format choisi
async function generateFactureNumber() {
    const format = companyInfo.factureFormat || 'FN';
    const now = new Date();
    ...
    try {
        ...
        switch (format) {
            case 'FYYYYMMN':
                factureNumber = `F${year}${month}${formattedNumber}`;
                break;
            case 'FYYYYN':
                factureNumber = `F${year}${formattedNumber}`;
                break;
            case 'FN':
                factureNumber = `F${formattedNumber}`;
                break;
            case 'FACTUREN':
                factureNumber = `FACTURE${formattedNumber}`;
                break;
            default:
                factureNumber = `F${formattedNumber}`;
        }
